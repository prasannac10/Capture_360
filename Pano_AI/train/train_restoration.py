...
from train.restoration import evaluate, train_one_epoch
...
def main():
    ...
    ds = CorrectionPairDataset(args.data, args.stage, tuple(cfg.get("input_size", [256, 512])))
    if len(ds) < 2:
        raise RuntimeError(f"Need paired intermediate artifacts for {args.stage}; found {len(ds)} pairs")
    ...
    model.to(device)
    optimizer = torch.optim.AdamW(model.parameters(), lr=cfg.get("lr", 1e-4), weight_decay=cfg.get("weight_decay", 1e-4))
    ...
    for epoch in range(cfg.get("epochs", 20)):
        ...
        print(f"epoch {epoch + 1} | train={loss:.6f} | val={metrics['loss']:.6f} | psnr={metrics['psnr']:.3f} | ssim={metrics['ssim']:.4f}")
        if metrics["loss"] < best:
            best = metrics["loss"]
            torch.save({"model": model.state_dict(), "optimizer": optimizer.state_dict(), "epoch": epoch + 1, "metrics": metrics}, os.path.join(args.out, f"{args.stage}_best.pt"))
