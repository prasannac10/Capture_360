...
class MainActivity : AppCompatActivity() {
    ...
    private val pinholeProfile=CameraProfile(CameraProjection.PINHOLE,fxNorm=1.0f,fyNorm=1.0f,cxNorm=0.0f,cyNorm=0.0f)
    ...
    private fun resetCaptureState(){
        capturedFrames.forEach{it.delete()}; capturedFrames.clear(); capturedPoses.clear(); lastCaptureYaw=0f; lastCapturePitch=0f
        if(::tracker.isInitialized) tracker=AngularCoverageTracker(10,3)
        btnFinish.visibility=View.GONE; progressBar.progress=0; updateProfileUi()
    }
    private fun updateProfileUi(){
        val fisheye=cameraProjection==CameraProjection.FISHEYE_180
        cameraProfileSwitch.text=if(fisheye)"Fisheye 180°" else "Pinhole / Phone"
        statusText.text=if(fisheye)"Fisheye: capture 4–8 views (target 6)" else "Phone: capture 20–30 views"
    }
    ...
    private fun startCaptureLogic(){
        try{
            camera=CameraController(this,this); fusion=SensorFusionManager(this); tracker=AngularCoverageTracker(10,3); resetCaptureState();
            getExternalFilesDir(null)?.listFiles()?.filter{it.name.startsWith("capture_")}?.forEach{it.delete()}; camera.startCamera(preview); fusion.start()
            lifecycleScope.launch{
                while(isActive){
                    try{
                        val o=fusion.getOrientation(); val yaw=Math.toDegrees(o.yaw.toDouble()).toFloat(); val pitch=Math.toDegrees(o.pitch.toDouble()).toFloat(); val rem=tracker.getRemainingCount(); val count=capturedFrames.size; val target=currentTargetFrames(); val max=currentMaxFrames()
                        overlay.updateDisplay(tracker.getGrid(),"${tracker.getTargetDirection(yaw,pitch,getScreenRotation())}\n($count/$target Captured)"); progressBar.progress=((count.toFloat()/target)*100).toInt().coerceAtMost(100)
                        if(count>=currentMinFrames())btnFinish.visibility=View.VISIBLE
                        val yd=calculateYawDiff(yaw,lastCaptureYaw); val pd=abs(pitch-lastCapturePitch)
                        if(count<max&&!isCameraBusy&&rem>0&&gate.shouldCapture(o)&&!tracker.isCurrentAreaCovered(o)&&sqrt(yd.toDouble().pow(2.0)+pd.toDouble().pow(2.0))>8f)triggerImageCapture(yaw,pitch,o)
                    }catch(e:Exception){Log.e("ControlLoop","Capture loop error",e)}; delay(100)
                }
            }
        }catch(e:Exception){Log.e("MainActivity","Camera startup failed",e);statusText.text="Error: ${e.localizedMessage}"}
    }
    private fun triggerImageCapture(yaw:Float,pitch:Float,o:Orientation){
        if(isCameraBusy)return; isCameraBusy=true; val dir=getExternalFilesDir(null)?:run{isCameraBusy=false;return}; val file=File(dir,"capture_${System.currentTimeMillis()}.jpg")
        camera.captureFrame(file){if(file.exists()&&file.length()>0){capturedFrames+=file;capturedPoses+=o;tracker.update(o);runOnUiThread{lastCaptureYaw=yaw;lastCapturePitch=pitch;statusText.text="${if(cameraProjection==CameraProjection.FISHEYE_180)"Fisheye" else "Phone"} captured: ${capturedFrames.size}/${currentTargetFrames()}"}};isCameraBusy=false}
    }
    private fun initiateStitching(){
        val dir=getExternalFilesDir(null)?:run{statusText.text="Error: Cannot access capture directory";return}; val n=capturedFrames.size
        if(n<currentMinFrames()){statusText.text="Need at least ${currentMinFrames()} images to stitch (found $n)";return}; if(n!=capturedPoses.size){statusText.text="Error: frame/pose count mismatch";return}
        btnFinish.isEnabled=false; modeSwitch.isEnabled=false; cameraProfileSwitch.isEnabled=false; progressBar.isIndeterminate=true
        val ai=modeSwitch.isChecked; val stitcher:PanoramaStitcher=if(ai)aiStitcher else classicalStitcher; val profile=currentProfile()
        statusText.text="${if(ai)"AI" else "OpenCV"} ${if(profile.projection==CameraProjection.FISHEYE_180)"fisheye" else "pinhole"} stitching $n images..."
        lifecycleScope.launch(Dispatchers.Default){var pano:Mat?=null;try{pano=stitcher.stitch(capturedFrames.toList(),capturedPoses.toList(),profile).getOrElse{throw it};pano=correctionChain.apply(pano!!);val file=File(dir,"panorama_result.jpg");if(!Imgcodecs.imwrite(file.absolutePath,pano))throw IllegalStateException("Failed to write panorama_result.jpg");val saved=saveImageToGallery(file,"panorama_${System.currentTimeMillis()}.jpg");withContext(Dispatchers.Main){statusText.text="Success! Panorama saved${if(saved)" to gallery ✓" else ""}"}}catch(e:Throwable){withContext(Dispatchers.Main){statusText.text=e.message?:"Stitching failed";Toast.makeText(this@MainActivity,statusText.text,Toast.LENGTH_LONG).show()}}finally{pano?.release();withContext(Dispatchers.Main){btnFinish.isEnabled=true;modeSwitch.isEnabled=true;cameraProfileSwitch.isEnabled=true;progressBar.isIndeterminate=false;progressBar.progress=100}}}
    }
