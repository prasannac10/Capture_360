...
def main():
    p=argparse.ArgumentParser(); p.add_argument('--config',default='../stitching/config.yaml'); p.add_argument('--epochs',type=int); a=p.parse_args(); root=Path(__file__).resolve().parent; cfg=yaml.safe_load((root/a.config).read_text()); m=cfg['model']; tr=cfg['training']; dev=torch.device('cuda' if torch.cuda.is_available() else 'cpu'); tile_bs=int(tr.get('tile_batch_size',cfg.get('inference',{}).get('tile_batch_size',4)))
