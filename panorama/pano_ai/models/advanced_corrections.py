...
from .restoration_backbone import RestorationUNet, ConvBlock
...
class ParallaxCorrectionUNet(RestorationUNet):
    ...
    def __init__(
        self, in_channels: int = 3, out_channels: int = 3, base_channels: int = 32
    ):
        """
...
class SeamBlendingUNet(nn.Module):
    ...
    def forward(
        self, img1: torch.Tensor, img2: torch.Tensor, seam_edge: torch.Tensor
    ) -> torch.Tensor:
        """
        ...
        d2 = self.decoder[2](torch.cat([self.decoder[1](d3), e2], dim=1))

        d1 = self.decoder[4](torch.cat([self.decoder[3](d2), e1], dim=1))
...
class ParallaxCorrectionDetector(nn.Module):
    ...
    def forward(
        self, img1: torch.Tensor, img2: torch.Tensor, overlap_mask: torch.Tensor = None
    ) -> torch.Tensor:
        """
