...
def save_checkpoint(path, model, optimizer=None, epoch=None, ema=None):
    ...
    if optimizer is not None:
        x["optimizer"] = optimizer.state_dict()
...
def load_checkpoint(path, model, device="cpu", ema=None, use_ema=False, optimizer=None):
    ...
    model.load_state_dict(x.get("model", x), strict=True)
    if optimizer is not None and "optimizer" in x:
        optimizer.load_state_dict(x["optimizer"])
    if ema is not None and "ema" in x:
        ema.shadow = {k: v.to(device) for k, v in x["ema"].items()}
        ema.copy_to(model) if use_ema else None
    return x
