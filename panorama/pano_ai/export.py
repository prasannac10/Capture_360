...
def main():
    ...
    p.add_argument("--output", default="artifacts/tiled_runtime_contract.json")
    a = p.parse_args()
    out = Path(a.output)
    ...
    out.write_text(
        json.dumps(
            {
                "input": "native-resolution frames",
                "tiling": "1024x1024 with 128 overlap",
                "frame_range": [4, 30],
                "projections": ["fisheye_180", "pinhole"],
                "output": [12000, 6000],
                "runtime_status": "python-reference",
                "onnx_status": "not_exported_for_native_tiled_path",
            },
            indent=2,
        ),
        encoding="utf-8",
